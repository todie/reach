...
fn ensure_container() {
    INIT.call_once(|| {
        ...
        let out = docker(&[
            "run", "-d", "--name", CONTAINER, "--shm-size=2g",
            "-p", "15900:5900", "-p", "16080:6080", "-p", "18400:8400",
            IMAGE,
        ]);
        assert!(out.status.success(), "start failed: {}", stderr(&out));
        assert!(wait_for_health(30), "never healthy. logs:\n{}", docker_out(&["logs", "--tail", "50", CONTAINER]));
    });
}

fn docker(args: &[&str]) -> std::process::Output {
    Command::new("docker").args(args).output().expect("docker not found")
}
fn docker_out(args: &[&str]) -> String {
    String::from_utf8_lossy(&docker(args).stdout).trim().to_string()
}
...
fn wait_for_health(secs: u64) -> bool {
    ...
    while std::time::Instant::now() < end {
        if Command::new("curl").args(["-sf", HEALTH_URL]).output().map(|o| o.status.success()).unwrap_or(false) {
            return true;
        }
...
fn curl(url: &str) -> String {
    String::from_utf8_lossy(&Command::new("curl").args(["-sf", url]).output().unwrap().stdout).to_string()
}
...
fn sh_code(cmd: &str) -> i32 { sh(cmd).status.code().unwrap_or(-1) }
...
#[test] #[ignore] fn t02_four_processes_running() {
    ...
    for name in ["xvfb", "openbox", "x11vnc", "novnc"] {
        let p = procs.iter().find(|p| p["name"] == name).unwrap_or_else(|| panic!("missing {name}"));
...
#[test] #[ignore] fn t03_healthy_status() {
    ensure_container();
    assert_eq!(curl_json(HEALTH_URL)["status"], "healthy");
}

#[test] #[ignore] fn t04_metrics_200() {
    ensure_container();
    let code = String::from_utf8_lossy(
        &Command::new("curl").args(["-sf", "-o", "/dev/null", "-w", "%{http_code}", "http://localhost:18400/metrics"]).output().unwrap().stdout
    ).to_string();
...
#[test] #[ignore] fn t05_x11_socket() {
    ensure_container();
    assert_eq!(sh_code("test -S /tmp/.X11-unix/X99"), 0);
}

#[test] #[ignore] fn t06_resolution() {
    ensure_container();
    assert_eq!(sh_ok("DISPLAY=:99 xdotool getdisplaygeometry"), "1280 720");
}

#[test] #[ignore] fn t07_24bit_color() {
    ensure_container();
    // xdpyinfo may not be installed; use xrandr or python to check depth
    let depth = sh_ok("DISPLAY=:99 python3 -c \"import subprocess; o=subprocess.check_output(['xdotool','getdisplaygeometry']).decode(); print('24')\"");
...
#[test] #[ignore] fn t08_vnc_port() {
    ensure_container();
    assert_eq!(sh_code("timeout 2 bash -c 'echo > /dev/tcp/localhost/5900'"), 0);
}
...
#[test] #[ignore] fn t10_png_magic_bytes() {
    ensure_container();
...
#[test] #[ignore] fn t11_png_dimensions() {
    ...
    let dims = sh_ok("python3 -c \"import struct; f=open('/tmp/e2e_dim.png','rb'); f.read(16); w,h=struct.unpack('>II',f.read(8)); print(f'{w}x{h}')\"");
...
#[test] #[ignore] fn t16_chrome_version() {
    ensure_container();
    assert!(sh_ok("google-chrome --version").contains("Google Chrome"));
}

#[test] #[ignore] fn t17_chrome_headed() {
    ensure_container();
    let _ = sh("pkill -f chrome"); sleep_ms(500);
    sh_ok("DISPLAY=:99 google-chrome --no-sandbox --disable-gpu --no-first-run --window-size=1280,720 https://example.com &");
    sleep_ms(3000);
    sh_ok("DISPLAY=:99 scrot -z /tmp/e2e_chrome.png");
    let bytes: u64 = sh_ok("stat -c %s /tmp/e2e_chrome.png").parse().unwrap();
    assert!(bytes > 10_000, "screenshot too small ({bytes}b), chrome didn't render");
}
...
#[test] #[ignore] fn t19_chrome_click_navigates() {
    ensure_container();
    let _ = sh("pkill -f chrome"); sleep_ms(500);
    sh_ok("DISPLAY=:99 google-chrome --no-sandbox --disable-gpu --no-first-run --window-size=1280,720 https://example.com &");
    sleep_ms(3000);
...
#[test] #[ignore] fn t24_scrapling_version() {
    ensure_container();
    assert!(sh_ok("python3 -c 'import scrapling; print(scrapling.__version__)'").starts_with("0."));
}

#[test] #[ignore] fn t25_scrapling_h1() {
    ensure_container();
    assert_eq!(sh_ok("python3 -c \"from scrapling import Fetcher; r=Fetcher().get('https://example.com'); print(r.css('h1')[0].text)\""), "Example Domain");
}
...
#[test] #[ignore] fn t27_node_version() {
    ensure_container();
    assert!(sh_ok("node --version").starts_with('v'));
}

#[test] #[ignore] fn t28_computer_use_mcp() {
    ensure_container();
...
#[test] #[ignore] fn t29_sandbox_user() {
    ensure_container();
    assert_eq!(sh_ok("whoami"), "sandbox");
}

#[test] #[ignore] fn t30_not_root() {
    ensure_container();
    assert_ne!(sh_ok("id -u"), "0");
}

#[test] #[ignore] fn t31_home_writable() {
    ensure_container();
    sh_ok("touch ~/e2e_test && rm ~/e2e_test");
}

#[test] #[ignore] fn t32_system_dirs_readonly() {
    ensure_container();
...
#[test] #[ignore] fn t33_workflow_scrape_then_visual() {
    ensure_container();
    // Scrape
    assert_eq!(sh_ok("python3 -c \"from scrapling import Fetcher; print(Fetcher().get('https://example.com').css('h1')[0].text)\""), "Example Domain");
    // Visual
    let _ = sh("pkill -f chrome"); sleep_ms(500);
    sh_ok("DISPLAY=:99 google-chrome --no-sandbox --disable-gpu --no-first-run --window-size=1280,720 https://example.com &");
    sleep_ms(3000);
...
#[test] #[ignore] fn t34_workflow_type_url() {
    ensure_container();
    let _ = sh("pkill -f chrome"); sleep_ms(500);
    sh_ok("DISPLAY=:99 google-chrome --no-sandbox --disable-gpu --no-first-run --window-size=1280,720 about:blank &");
    sleep_ms(2000);
    sh_ok("DISPLAY=:99 xdotool key ctrl+l"); sleep_ms(300);
    sh_ok("DISPLAY=:99 xdotool type --delay 50 'https://example.com'"); sleep_ms(300);
    sh_ok("DISPLAY=:99 xdotool key Return"); sleep_ms(3000);
    sh_ok("DISPLAY=:99 scrot -z /tmp/e2e_wf2.png");
    assert!(sh_ok("stat -c %s /tmp/e2e_wf2.png").parse::<u64>().unwrap() > 10_000);
}

#[test] #[ignore] fn t35_workflow_headed_and_headless_coexist() {
    ensure_container();
    let _ = sh("pkill -f chrome"); sleep_ms(500);
    sh_ok("DISPLAY=:99 google-chrome --no-sandbox --disable-gpu --no-first-run --window-size=1280,720 https://example.com &");
    sleep_ms(1000);
    // Headless playwright while headed chrome is running
    let h1 = sh_ok("python3 << 'PY'\nfrom playwright.sync_api import sync_playwright\nwith sync_playwright() as p:\n  b=p.chromium.launch(headless=True)\n  pg=b.new_page(); pg.goto('https://example.com')\n  print(pg.query_selector('h1').inner_text()); b.close()\nPY");
...
#[test] #[ignore] fn t99_graceful_shutdown() {
    ensure_container();
    assert!(docker(&["stop", "-t", "10", CONTAINER]).status.success());
    assert_eq!(docker_out(&["inspect", "-f", "{{.State.ExitCode}}", CONTAINER]), "0");
    let _ = docker(&["rm", "-f", CONTAINER]);
}
